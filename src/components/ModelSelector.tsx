...
export default function ModelSelector({ models, activeModel, onChange }: Props) {
  ...
  const availableCount = models.filter(m => m.disponible).length
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="text-slate-400 ml-1 font-mono text-xs">R2={activeModel.r2.toFixed(3)}</span>
        ...
        <div className="absolute right-0 top-full mt-2 w-[460px] max-h-[70vh] overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-xl z-[9999]">
          <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 sticky top-0">
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              {models.length} modelos oficiales · {availableCount} con serie/mapa cargado · metricas H4 finales
            </p>
          </div>
          <div className="divide-y divide-slate-100">
            {ordered.map(m => {
              const active = m.id === activeModel.id
              const disabled = !m.disponible
              return (
                <button
                  key={m.id}
                  disabled={disabled}
                  onClick={() => { if (!disabled) { onChange(m); setOpen(false) } }}
                  className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-colors ${
                    active ? 'bg-violet-50' : disabled ? 'opacity-60 cursor-not-allowed' : 'hover:bg-slate-50'
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                        ...
                        {m.map
                          ...
                          : m.scope === 'nacional'
                            ...
                            : m.disponible
                              ...
                              : <span className="text-[8px] font-bold px-1.5 py-0.5 rounded bg-slate-100 text-slate-400">SOLO METRICA</span>}
                    ...
                    <div className="flex gap-4 mt-1.5">
                      {m.n != null && <Metric label="n" value={m.n.toLocaleString()} good />}
                      <Metric label="R2" value={m.r2.toFixed(3)} good={m.r2 >= 0.5} />
                      <Metric label="RMSE" value={m.rmse.toLocaleString(undefined, { maximumFractionDigits: 2 })} good={m.rmse < 12} />
